...
extension Grip {
        ...
        var body: some View {
            ...
            ZStack {
                if (!surfaceView.inspectorVisible) {
                    ...
                    SplitView(.vertical, $split, dividerColor: grip.config.splitDividerColor, left: {
                        SurfaceWrapper(surfaceView: surfaceView, isSplit: isSplit)
                    }, right: {
    ...
    class InspectorView: MTKView, NSTextInputClient {
        ...
        private var inspector: Grip.Inspector? {
        ...
        private func keyAction(_ action: ghostty_input_action_e, event: NSEvent) {
            guard let inspector = self.inspector else { return }
            guard let key = Grip.Input.Key(keyCode: event.keyCode) else { return }
            let mods = Grip.gripMods(event.modifierFlags)

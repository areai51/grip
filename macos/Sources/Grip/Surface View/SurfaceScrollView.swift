...
/// Wraps a Grip surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Grip renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: Grip.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: Grip.SurfaceView) {
        ...
        // so that our primary Grip renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Grip
        observers.append(NotificationCenter.default.addObserver(
            forName: .gripDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? Grip.Action.Scrollbar else {
            return
        }

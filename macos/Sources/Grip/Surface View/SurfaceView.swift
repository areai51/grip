...
extension Grip {
    ...
    struct Terminal: View {
        @EnvironmentObject private var grip: Grip.App

        var body: some View {
            if let app = self.grip.app {
    ...
    /// Yields a SurfaceView for a grip app that can then be used however you want.
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    SurfaceRepresentable(view: surfaceView, size: geo.size)
                        ...
                        .focusedValue(\.gripSurfacePwd, surfaceView.pwd)
                        .focusedValue(\.gripSurfaceView, surfaceView)
                        .focusedValue(\.gripSurfaceCellSize, surfaceView.cellSize)
                    #if canImport(AppKit)
                    ...
                    if let surfaceSize = surfaceView.surfaceSize {
                        SurfaceResizeOverlay(
                            geoSize: geo.size,
                            size: surfaceSize,
                            overlay: grip.config.resizeOverlay,
                            position: grip.config.resizeOverlayPosition,
                            duration: grip.config.resizeOverlayDuration,
                            focusInstant: surfaceView.focusInstant)

                    }
                }
                .gripSurfaceView(surfaceView)
                ...
                if (grip.config.secureInputIndication &&
                    secureInput.enabled &&
                    surfaceFocus &&
                    windowFocus) {
                    SecureInputOverlay()
                ...
                if (grip.config.bellFeatures.contains(.border)) {
                ...
                if (!surfaceView.healthy) {
                    Rectangle().fill(grip.config.backgroundColor)
                ...
                if (isSplit && !surfaceFocus) {
                    let overlayOpacity = grip.config.unfocusedSplitOpacity;
                    if (overlayOpacity > 0) {
                        Rectangle()
                            .fill(grip.config.unfocusedSplitFill)
                            .allowsHitTesting(false)
    ...
    struct SurfaceResizeOverlay: View {
        ...
        let overlay: Grip.Config.ResizeOverlay
        let position: Grip.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                ...
                .onReceive(NotificationCenter.default.publisher(for: .gripSearchFocus)) { notification in
    ...
    /// A surface is terminology in Grip for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libgrip, usually from the Grip configuration.
    struct SurfaceConfiguration {
        ...
        /// Provides a C-compatible grip configuration within a closure. The configuration
        ...
        func withCValue<T>(view: SurfaceView, _ body: (inout ghostty_surface_config_s) throws -> T) rethrows -> T {
            ...
            // libgrip APIs when a UIView is attached to a window/scene. TODO.
...
private struct GripSurfaceViewKey: EnvironmentKey {
    static let defaultValue: Grip.SurfaceView? = nil
}

extension EnvironmentValues {
    var gripSurfaceView: Grip.SurfaceView? {
        get { self[GripSurfaceViewKey.self] }
        set { self[GripSurfaceViewKey.self] = newValue }
    }
}

extension View {
    func gripSurfaceView(_ surfaceView: Grip.SurfaceView?) -> some View {
        environment(\.gripSurfaceView, surfaceView)
    }
}
...
extension FocusedValues {
    var gripSurfaceView: Grip.SurfaceView? {
        get { self[FocusedGripSurface.self] }
        set { self[FocusedGripSurface.self] = newValue }
    }

    struct FocusedGripSurface: FocusedValueKey {
        typealias Value = Grip.SurfaceView
    }

    var gripSurfacePwd: String? {
        get { self[FocusedGripSurfacePwd.self] }
        set { self[FocusedGripSurfacePwd.self] = newValue }
    }

    struct FocusedGripSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var gripSurfaceCellSize: OSSize? {
        get { self[FocusedGripSurfaceCellSize.self] }
        set { self[FocusedGripSurfaceCellSize.self] = newValue }
    }

    struct FocusedGripSurfaceCellSize: FocusedValueKey {
...
extension Grip.SurfaceView {
    class SearchState: ObservableObject {
        ...
        init(from startSearch: Grip.Action.StartSearch) {

...
extension Grip.SurfaceView: Transferable {
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(contentType: .gripSurfaceId) { surface in
    ...
    static func find(uuid: UUID) -> Self? {
        #if canImport(AppKit)
        guard let del = NSApp.delegate as? Grip.Delegate else { return nil }
        return del.gripSurface(id: uuid) as? Self
...
extension UTType {
    ...
    static let gripSurfaceId = UTType(exportedAs: "com.mitchellh.gripSurfaceId")
}

#if canImport(AppKit)
extension NSPasteboard.PasteboardType {
    /// Pasteboard type for dragging surface IDs.
    static let gripSurfaceId = NSPasteboard.PasteboardType(UTType.gripSurfaceId.identifier)
}
#endif

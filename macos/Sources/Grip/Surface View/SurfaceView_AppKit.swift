...
extension Grip {
    ...
    class SurfaceView: OSView, ObservableObject, Codable, Identifiable {
        ...
        // detected with the set_password_input_cb on the Grip state.
        ...
        private(set) var surfaceModel: Grip.Surface?
        ...
        var scrollbar: Grip.Action.Scrollbar?
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            if let appDelegate = NSApplication.shared.delegate as? AppDelegate {
                self.derivedConfig = DerivedConfig(appDelegate.grip.config)
            } else {
            ...
            center.addObserver(
                ...
                name: Grip.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(gripDidContinueKeySequence),
                name: Grip.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(gripDidEndKeySequence),
                name: Grip.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(gripDidChangeKeyTable),
                name: Grip.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(gripColorDidChange(_:)),
            ...
            center.addObserver(
                self,
                selector: #selector(gripDidChangeReadonly(_:)),
            ...
            guard let surface = surface else {
                self.error = Grip.Error.apiFailed
                return
            }
            self.surfaceModel = Grip.Surface(cSurface: surface)
        ...
        func sizeDidChange(_ size: CGSize) {
            // Grip wants to know the actual framebuffer size... It is very important
        ...
        @objc private func gripDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[Grip.Notification.KeySequenceKey] else { return }
        ...
        @objc private func gripDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func gripDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[Grip.Notification.KeyTableKey] as? Grip.Action.KeyTable else { return }
        ...
        @objc private func gripConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.GripConfigChangeKey
        ...
        @objc private func gripColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.GripColorChangeKey
            ] as? Grip.Action.ColorChange else { return }
        ...
        @objc private func gripBellDidRing(_ notification: SwiftUI.Notification) {
            // Bell state goes to true
            bell = true
        }

        @objc private func gripDidChangeReadonly(_ notification: SwiftUI.Notification) {
        ...
        @objc private func windowDidChangeScreen(notification: SwiftUI.Notification) {
            ...
            // When the window changes screens, we need to update libgrip with the screen
            ...
            // Issue: https://github.com/grip-org/grip/issues/2731
            DispatchQueue.main.async { [weak self] in
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Grip.gripMods(event.modifierFlags)
        ...
        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Grip.gripMods(event.modifierFlags)
            let button = Grip.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = Grip.Input.MousePosEvent(
                x: pos.x,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Grip first. We do this because this will let Grip handle
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsGrip = Grip.eventModifierFlags(
                mods: ghostty_surface_key_translation_mods(
                    surface,
                    Grip.gripMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsGrip directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if (translationModsGrip.contains(flag)) {
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.gripCharacters,
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                ...
                return (event.characters ?? "").withCString { ptr in
                    gripEvent.text = ptr
                    return surface.keyIsBinding(gripEvent)
                }
            }
        ...
        private func keyAction(
            ...
            var key_ev = event.gripKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by Grip itself.
        ...
        override func quickLook(with event: NSEvent) {
            ...
            // Grip coordinate system is top-left, convert to bottom-left for AppKit
        ...
        func showUserNotification(title: String, body: String) {
            ...
            content.categoryIdentifier = Grip.userNotificationCategory
        ...
        struct DerivedConfig {
            ...
            let backgroundBlur: Grip.Config.BackgroundBlur
            let macosWindowShadow: Bool
            let windowTitleFontFamily: String?
            let windowAppearance: NSAppearance?
            let scrollbar: Grip.Config.Scrollbar
            ...
            init(_ config: Grip.Config) {
                ...
                self.windowAppearance = .init(gripConfig: config)
...
extension Grip.SurfaceView: NSTextInputClient {
    ...
    func selectedRange() -> NSRange {
        ...
        // Get our range from the Grip API. There is a race condition between getting the
    ...
    func attributedSubstring(forProposedRange range: NSRange, actualRange: NSRangePointer?) -> NSAttributedString? {
        // Grip.logger.warning("pressure substring range=\(range) selectedRange=\(self.selectedRange())")
        guard let surface = self.surface else { return nil }
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Grip will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        // Grip coordinates are in top-left (0, 0) so we have to convert to
    ...
    /// Sync the preedit state based on the markedText value to libgrip
...
extension Grip.SurfaceView: NSServicesMenuRequestor {
...
extension Grip.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        switch item.action {
        case #selector(pasteSelection):
            let pb = NSPasteboard.gripSelection
...
extension Grip.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Grip core to
    /// expose that.
    ///

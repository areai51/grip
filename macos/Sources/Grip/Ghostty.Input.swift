...
extension Grip {
    ...
    /// because Grip input triggers are a superset of what can be represented by a macOS
    ...
    /// handling for Grip is handled at a lower level (usually). This function should generally only
    ...
    static func keyboardShortcut(for trigger: ghostty_input_trigger_s) -> KeyboardShortcut? {
        ...
        return KeyboardShortcut(
            key,
            modifiers: EventModifiers(nsFlags: Grip.eventModifierFlags(mods: trigger.mods)))
    }

    // MARK: Mods

    /// Returns the event modifier flags set for the Grip mods enum.
    ...
    /// Translate event modifier flags to a grip mods enum.
    static func gripMods(_ flags: NSEvent.ModifierFlags) -> ghostty_input_mods_e {
        ...
        // Grip structure but that's okay -- we don't use that information.
    ...
    /// A map from the Grip key enum to the keyEquivalent string for shortcuts. Note that
    /// not all grip key enum values are represented here because not all of them can be
...
// MARK: Grip.Input.BindingFlags

extension Grip.Input {
...
extension Grip.Input.Action: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Key Action")

    static var caseDisplayRepresentations: [Grip.Input.Action : DisplayRepresentation] = [
        .release: "Release",
        .press: "Press",
        .repeat: "Repeat"
    ]
...
extension Grip.Input.MouseState: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Mouse State")

    static var caseDisplayRepresentations: [Grip.Input.MouseState : DisplayRepresentation] = [
        .release: "Release",
        .press: "Press"
    ]
}

// MARK: Grip.Input.MouseButton
...
extension Grip.Input.MouseButton: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Mouse Button")

    static var caseDisplayRepresentations: [Grip.Input.MouseButton : DisplayRepresentation] = [
        .unknown: "Unknown",
        .left: "Left",
        .right: "Right",
        .middle: "Middle"
    ...
    static var allCases: [Grip.Input.MouseButton] = [
        .left,
        .right,
        .middle,
    ]
...
extension Grip.Input.Momentum: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Scroll Momentum")

    static var caseDisplayRepresentations: [Grip.Input.Momentum : DisplayRepresentation] = [
        .none: "None",
        .began: "Began",
...
extension Grip.Input {
    ...
    struct Mods: OptionSet {
        ...
        init(nsFlags: NSEvent.ModifierFlags) {
            self.init(cMods: Grip.gripMods(nsFlags))
        }

        var nsFlags: NSEvent.ModifierFlags {
            Grip.eventModifierFlags(mods: cMods)
        }
    }
}
...
extension Grip.Input.Key: AppEnum {
    ...
    static var allCases: [Grip.Input.Key] {
        return [
            // Letters (A-Z)
    ...
    static var caseDisplayRepresentations: [Grip.Input.Key : DisplayRepresentation] = [
        // Letters (A-Z)

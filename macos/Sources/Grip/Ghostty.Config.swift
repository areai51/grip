...
extension Grip {
    ...
    class Config: ObservableObject {
        // The underlying C pointer to the Grip config structure. This
        ...
        /// in the Grip configuration. For example `keybind = cmd+q=quit` in Grip
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            return Grip.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated Grip documentation for
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/grip/Grip.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>? = nil
...
#error("unsupported")
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromGripConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromGripConfig: str) ?? .move
        }
        ...
        var commandPaletteEntries: [Grip.Command] {
            guard let config = self.config else { return [] }
            ...
            return buffer.map { Grip.Command(cValue: $0) }
        }
    }
}

...
extension NSEvent {
    /// Create a Grip key event for a given keyboard action.
    ///
    ...
    func gripKeyEvent(
        ...
        key_ev.mods = Grip.gripMods(modifierFlags)
        key_ev.consumed_mods = Grip.gripMods(
    ...
    /// Returns the text to set for a key event for Grip.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within Grip.
    var gripCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within Grip's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to Grip.

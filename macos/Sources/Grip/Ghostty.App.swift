...
protocol GripAppDelegate: AnyObject {
    #if os(macOS)
...
extension Grip {
    ...
    // This is a refactor/redo of Grip.AppState so that it supports both macOS and iOS
    class App: ObservableObject {
        ...
        /// Optional delegate
        weak var delegate: GripAppDelegate?
        ...
        /// The grip app instance. We only have one of these for the entire app, although I guess
        ...
        init(configPath: String? = nil) {
            ...
            // Create our "runtime" config. The "runtime" is the configuration that grip
        ...
        static func openConfig() {
            let str = Grip.AllocatedString(ghostty_config_open_path()).string
        ...
        func reloadConfig(soft: Bool = false) {
            ...
            guard newConfig.loaded else {
                Grip.logger.warning("failed to reload configuration")
                return
            }
        ...
        // Called when the selected keyboard changes. We have to notify Grip so that
        ...
        // MARK: Grip Callbacks (macOS)

        static func closeSurface(_ userdata: UnsafeMutableRawPointer?, processAlive: Bool) {
            let surface = self.surfaceUserdata(from: userdata)
            NotificationCenter.default.post(name: Notification.gripCloseSurface, object: surface, userInfo: [
        ...
        static func readClipboard(_ userdata: UnsafeMutableRawPointer?, location: ghostty_clipboard_e, state: UnsafeMutableRawPointer?) {
            ...
            // Get our pasteboard
            guard let pasteboard = NSPasteboard.grip(location) else {
        ...
        static func confirmReadClipboard(
            ...
            guard let request = Grip.ClipboardRequest.from(request: request) else { return }
        ...
        static func writeClipboard(
            ...
            let surface = self.surfaceUserdata(from: userdata)
            guard let pasteboard = NSPasteboard.grip(location) else { return }
            guard let content = content, len > 0 else { return }
            
            // Convert the C array to Swift array
            let contentArray = (0..<len).compactMap { i in
                Grip.ClipboardContent.from(content: content[i])
            }
            ...
            NotificationCenter.default.post(
                ...
                userInfo: [
                    ...
                    Notification.ConfirmClipboardRequestKey: Grip.ClipboardRequest.osc_52_write(pasteboard),
                ]
            )
        }
        ...
        /// Determine if a given notification should be presented to the user when Grip is running in the foreground.
        ...
        /// Returns the GripState from the given userdata value.
        ...
        static func action(_ app: ghostty_app_t, target: ghostty_target_s, action: ghostty_action_s) -> Bool {
            ...
            default:
                Grip.logger.warning("unknown action target=\(target.tag.rawValue)")
                return false
            }
            ...
            case GHOSTTY_ACTION_SHOW_CHILD_EXITED:
                Grip.logger.info("known but unimplemented action action=\(action.tag.rawValue)")
                return false
            case GHOSTTY_ACTION_COPY_TITLE_TO_CLIPBOARD:
                return copyTitleToClipboard(app, target: target)
            default:
                Grip.logger.warning("unknown action action=\(action.tag.rawValue)")
                return false
            }
        ...
        private static func openURL(
            ...
            let action = Grip.Action.OpenURL(c: v)
            ...
            // See: https://github.com/grip-org/grip/issues/8763
            let url: URL
            ...
            switch action.kind {
            case .text:
                // Open with the default editor for `*.grip` file or just system text editor
        ...
        private static func newWindow(_ app: ghostty_app_t, target: ghostty_target_s) {
            ...
            case GHOSTTY_TARGET_APP:
                NotificationCenter.default.post(
                    name: Notification.gripNewWindow,
                    object: nil,
                    userInfo: [:]
                )
        ...
        private static func newSplit(
            ...
            case GHOSTTY_TARGET_APP:
                ...
                Grip.logger.warning("new split does nothing with an app target")
                return
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.gripNewSplit,
                    object: surfaceView,
                    userInfo: [
                        "direction": direction,
        ...
        private static func presentTerminal(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                NotificationCenter.default.post(
                    name: Notification.gripPresentTerminal,
                    object: surfaceView
                )
                return true
        ...
        private static func closeTab(_ app: ghostty_app_t, target: ghostty_target_s, mode: ghostty_action_close_tab_mode_e) {
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("close tabs does nothing with an app target")
                return
        ...
        private static func closeWindow(_ app: ghostty_app_t, target: ghostty_target_s) {
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("close window does nothing with an app target")
                return
        ...
        private static func toggleFullscreen(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle fullscreen does nothing with an app target")
                return

            case GHOSTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                guard let mode = FullscreenMode.from(grip: raw) else {
                    Grip.logger.warning("unknown fullscreen mode raw=\(raw.rawValue)")
                    return
                }
                NotificationCenter.default.post(
                    name: Notification.gripToggleFullscreen,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func toggleCommandPalette(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle command palette does nothing with an app target")
                return
        ...
        private static func toggleMaximize(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle maximize does nothing with an app target")
                return
        ...
        private static func ringBell(
            ...
            case GHOSTTY_TARGET_APP:
                ...
                Grip.logger.warning("ring bell does nothing with an app target")
                return
        ...
        private static func setReadonly(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("set readonly does nothing with an app target")
                return
        ...
        private static func moveTab(
            ...
            move: ghostty_action_move_tab_s) -> Bool {
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("move tab does nothing with an app target")
                    return false
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.GripMoveTabKey: Action.MoveTab(c: move),
                        ]
                    )
        ...
        private static func gotoTab(
            ...
            tab: ghostty_action_goto_tab_e) -> Bool {
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("goto tab does nothing with an app target")
                    return false
        ...
        private static func gotoSplit(
            ...
            direction: ghostty_action_goto_split_e) -> Bool {
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("goto split does nothing with an app target")
                    return false
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    let focusDirection: SplitTree<Grip.SurfaceView>.FocusDirection = splitDirection.toSplitTreeFocusDirection()
                    ...
                    NotificationCenter.default.post(
                        name: Notification.gripFocusSplit,
                        object: surfaceView,
                        userInfo: [
        ...
        private static func resizeSplit(
            ...
            resize: ghostty_action_resize_split_s) -> Bool {
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("resize split does nothing with an app target")
                    return false
        ...
        private static func equalizeSplits(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("equalize splits does nothing with an app target")
                return
        ...
        private static func toggleSplitZoom(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle split zoom does nothing with an app target")
                return false
        ...
        private static func controlInspector(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle inspector does nothing with an app target")
                return
        ...
        private static func showDesktopNotification(
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                center.requestAuthorization(options: [.alert, .sound]) { _, error in
                    if let error = error {
                        Grip.logger.error("Error while requesting notification authorization: \(error)")
                    }
                }
        ...
        private static func toggleFloatWindow(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle float window does nothing with an app target")
                return
        ...
        private static func toggleBackgroundOpacity(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("toggle background opacity does nothing with an app target")
                return
        ...
        private static func setTitle(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("set title does nothing with an app target")
                return
        ...
        private static func promptTitle(
            ...
            case .surface:
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("set title prompt does nothing with an app target")
                    return false
        ...
        private static func pwdChanged(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("pwd change does nothing with an app target")
                return
        ...
        private static func setMouseShape(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("set mouse shapes nothing with an app target")
                return
        ...
        private static func setMouseOverLink(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("mouse over link does nothing with an app target")
                return
        ...
        private static func setInitialSize(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("initial size does nothing with an app target")
                return
        ...
        private static func resetWindowSize(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("reset window size does nothing with an app target")
                return
        ...
        private static func keySequence(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("key sequence does nothing with an app target")
                return
        ...
        private static func keyTable(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("key table does nothing with an app target")
                return

            case GHOSTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                guard let surfaceView = self.surfaceView(from: surface) else { return }
                guard let action = Grip.Action.KeyTable(c: v) else { return }
        ...
        private static func progressReport(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("progress report does nothing with an app target")
                return
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                let progressReport = Grip.Action.ProgressReport(c: v)
        ...
        private static func scrollbar(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("scrollbar does nothing with an app target")
                return
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                let scrollbar = Grip.Action.Scrollbar(c: v)
                NotificationCenter.default.post(
                    name: .gripDidUpdateScrollbar,
                    object: surfaceView,
                    userInfo: [
        ...
        private static func startSearch(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("start_search does nothing with an app target")
                return
            ...
            case GHOSTTY_TARGET_SURFACE:
                ...
                let startSearch = Grip.Action.StartSearch(c: v)
                DispatchQueue.main.async {
                    if let searchState = surfaceView.searchState {
                        ...
                        surfaceView.searchState = Grip.SurfaceView.SearchState(from: startSearch)
                    }
                                        
                    NotificationCenter.default.post(name: .gripSearchFocus, object: surfaceView)
                }
        ...
        private static func endSearch(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("end_search does nothing with an app target")
                return
        ...
        private static func searchTotal(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("search_total does nothing with an app target")
                return
        ...
        private static func searchSelected(
            ...
            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                Grip.logger.warning("search_selected does nothing with an app target")
                return
            ...
            v: ghostty_action_reload_config_s)
        {
            ...
            let grip = Unmanaged<App>.fromOpaque(app_ud).takeUnretainedValue()

            switch (target.tag) {
            case GHOSTTY_TARGET_APP:
                grip.reloadConfig(soft: v.soft)
                return

            case GHOSTTY_TARGET_SURFACE:
                guard let surface = target.target.surface else { return }
                grip.reloadConfig(surface: surface, soft: v.soft)
            ...
            v: ghostty_action_config_change_s) {
                ...
                case GHOSTTY_TARGET_APP:
                    ...
                    NotificationCenter.default.post(
                        ...
                        object: nil,
                        userInfo: [
                            SwiftUI.Notification.Name.GripConfigChangeKey: config,
                        ]
                    )
        ...
        private static func colorChange(
            ...
            change: ghostty_action_color_change_s) {
                switch (target.tag) {
                case GHOSTTY_TARGET_APP:
                    Grip.logger.warning("color change does nothing with an app target")
                    return
                ...
                case GHOSTTY_TARGET_SURFACE:
                    ...
                    guard let surfaceView = self.surfaceView(from: surface) else { return }
                    NotificationCenter.default.post(
                        name: .gripColorDidChange,
                        object: surfaceView,
                        userInfo: [
                            SwiftUI.Notification.Name.GripColorChangeKey: Action.ColorChange(c: change)
                        ]
                    )
        ...
        func handleUserNotification(response: UNNotificationResponse) {
            ...
            case UNNotificationDefaultActionIdentifier, Grip.userNotificationActionShow:

...
struct CommandEntity: AppEntity {
    ...
    init(_ command: Grip.Command, for terminal: TerminalEntity) {
...
struct CommandQuery: EntityQuery {
    ...
    func entities(for identifiers: [CommandEntity.ID]) async throws -> [CommandEntity] {
        ...
        let commands = appDelegate.grip.config.commandPaletteEntries
    ...
    func suggestedEntities() async throws -> [CommandEntity] {
        ...
        return appDelegate.grip.config.commandPaletteEntries.map { CommandEntity($0, for: terminal) }
    }
}

...
struct NewTerminalIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ReturnsValue<TerminalEntity?> {
        ...
        guard let appDelegate = NSApp.delegate as? AppDelegate else {
            throw GripIntentError.appUnavailable
        }
        let grip = appDelegate.grip
        ...
        if let command {
            config.initialInput = "\(Grip.Shell.quote(command)); exit\n"
        }
...
enum NewTerminalLocation: String {
    ...
    var splitDirection: SplitTree<Grip.SurfaceView>.NewDirection? {
        switch self {

...
/// A base class for windows that can contain Grip windows. This base class implements
/// the bare minimum functionality that every terminal window in Grip should implement.
///
                              ...
                              FullscreenDelegate
{
    ...
    var focusedSurface: Grip.SurfaceView? = nil {
        didSet { syncFocusToSurfaceTree() }
    }

    /// The tree of splits within this terminal window.
    @Published var surfaceTree: SplitTree<Grip.SurfaceView> = .init() {
    ...
    init(_ grip: Grip.App,
         baseConfig base: Grip.SurfaceConfiguration? = nil,
         surfaceTree tree: SplitTree<Grip.SurfaceView>? = nil
    ) {
        self.grip = grip
        ...
        guard let ghostty_app = grip.app else { preconditionFailure("app must be loaded") }
        self.surfaceTree = tree ?? .init(view: Grip.SurfaceView(ghostty_app, baseConfig: base))
        ...
        center.addObserver(
            ...
            name: Grip.Notification.confirmClipboard,
            object: nil)
        center.addObserver(
            self,
        ...
        center.addObserver(
            self,
            selector: #selector(gripConfigDidChangeBase(_:)),
            name: .gripConfigDidChange,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripCommandPaletteDidToggle(_:)),
            name: .gripCommandPaletteDidToggle,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripMaximizeDidToggle(_:)),
            name: .gripMaximizeDidToggle,
            object: nil)

        // Splits
        center.addObserver(
            self,
            selector: #selector(gripDidCloseSurface(_:)),
            name: Grip.Notification.gripCloseSurface,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidNewSplit(_:)),
            name: Grip.Notification.gripNewSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidEqualizeSplits(_:)),
            name: Grip.Notification.didEqualizeSplits,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidFocusSplit(_:)),
            name: Grip.Notification.gripFocusSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidToggleSplitZoom(_:)),
            name: Grip.Notification.didToggleSplitZoom,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidResizeSplit(_:)),
            name: Grip.Notification.didResizeSplit,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripDidPresentTerminal(_:)),
            name: Grip.Notification.gripPresentTerminal,
            object: nil)
        center.addObserver(
            self,
            selector: #selector(gripSurfaceDragEndedNoTarget(_:)),
            name: .gripSurfaceDragEndedNoTarget,
            object: nil)
    ...
    @discardableResult
    func newSplit(
        at oldView: Grip.SurfaceView,
        direction: SplitTree<Grip.SurfaceView>.NewDirection,
        baseConfig config: Grip.SurfaceConfiguration? = nil
    ) -> Grip.SurfaceView? {
        // We can only create new splits for surfaces in our tree.
        guard surfaceTree.root?.node(view: oldView) != nil else { return nil }

        // Create a new surface view
        guard let ghostty_app = grip.app else { return nil }
        let newView = Grip.SurfaceView(ghostty_app, baseConfig: config)

        // Do the split
        let newTree: SplitTree<Grip.SurfaceView>
        do {
            ...
            // no big deal.
            Grip.logger.warning("failed to insert split: \(error)")
            return nil
        }
    ...
    func surfaceTreeDidChange(from: SplitTree<Grip.SurfaceView>, to: SplitTree<Grip.SurfaceView>) {
    ...
    /// Update all surfaces with the focus state. This ensures that libgrip has an accurate view about
    ...
    func closeSurface(
        ...
        // due to SwiftUI bugs (see Grip #560). To repeat from #560, the bug is that
    ...
    private func findNextFocusTargetAfterClosing(node: SplitTree<Grip.SurfaceView>.Node) -> Grip.SurfaceView? {
    ...
    private func removeSurfaceNode(_ node: SplitTree<Grip.SurfaceView>.Node) {
        // Move focus if the closed surface was focused and we have a next target
        let nextFocus: Grip.SurfaceView? = if node.contains(
    ...
    func replaceSurfaceTree(
        ...
        if let newView {
            DispatchQueue.main.async {
                Grip.moveFocus(to: newView, from: oldView)
            }
        }
        ...
        undoManager.registerUndo(
            ...
            if let oldView {
                DispatchQueue.main.async {
                    Grip.moveFocus(to: oldView, from: target.focusedSurface)
                }
            }
    ...
    @objc private func gripConfigDidChangeBase(_ notification: Notification) {
    ...
    @objc private func gripCommandPaletteDidToggle(_ notification: Notification) {
        guard let surfaceView = notification.object as? Grip.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        toggleCommandPalette(nil)
    }

    @objc private func gripMaximizeDidToggle(_ notification: Notification) {
        guard let window else { return }
        guard let surfaceView = notification.object as? Grip.SurfaceView else { return }
        guard surfaceTree.contains(surfaceView) else { return }
        window.zoom(nil)
    ...
    @objc private func gripDidCloseSurface(_ notification: Notification) {
    ...
    @objc private func gripDidNewSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let oldView = notification.object as? Grip.SurfaceView else { return }
        ...
        let splitDirection: SplitTree<Grip.SurfaceView>.NewDirection
        switch (direction) {
    ...
    @objc private func gripDidEqualizeSplits(_ notification: Notification) {
        guard let target = notification.object as? Grip.SurfaceView else { return }
    ...
    @objc private func gripDidFocusSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Grip.SurfaceView else { return }
        guard surfaceTree.root?.node(view: target) != nil else { return }

        // Get the direction from the notification
        guard let directionAny = notification.userInfo?[Grip.Notification.SplitDirectionKey] else { return }
        guard let direction = directionAny as? Grip.SplitFocusDirection else { return }
        ...
        DispatchQueue.main.async {
            Grip.moveFocus(to: nextSurface, from: target)
        }
    }
    
    @objc private func gripDidToggleSplitZoom(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Grip.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        ...
        DispatchQueue.main.async {
            Grip.moveFocus(to: target)
        }
    }
    
    @objc private func gripDidResizeSplit(_ notification: Notification) {
        // The target must be within our tree
        guard let target = notification.object as? Grip.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        
        // Extract direction and amount from notification
        guard let directionAny = notification.userInfo?[Grip.Notification.ResizeSplitDirectionKey] else { return }
        guard let direction = directionAny as? Grip.SplitResizeDirection else { return }
        
        guard let amountAny = notification.userInfo?[Grip.Notification.ResizeSplitAmountKey] else { return }
        guard let amount = amountAny as? UInt16 else { return }
        
        // Convert Grip.SplitResizeDirection to SplitTree.Spatial.Direction
        let spatialDirection: SplitTree<Grip.SurfaceView>.Spatial.Direction
        switch direction {
        ...
        do {
            ...
            Grip.logger.warning("failed to resize split: \(error)")
        }
    }

    @objc private func gripDidPresentTerminal(_ notification: Notification) {
        guard let target = notification.object as? Grip.SurfaceView else { return }
        guard surfaceTree.contains(target) else { return }
        ...
        Grip.moveFocus(to: target)
        Grip.moveFocus(to: target, delay: 0.1)

        // Show a brief highlight to help the user locate the presented terminal.
        target.highlight()
    }

    @objc private func gripSurfaceDragEndedNoTarget(_ notification: Notification) {
        guard let target = notification.object as? Grip.SurfaceView else { return }
        guard let targetNode = surfaceTree.root?.node(view: target) else { return }
        ...
        let newTree = SplitTree<Grip.SurfaceView>(view: target)
        ...
        _ = TerminalController.newWindow(
            grip,
            tree: newTree,
            position: notification.userInfo?[Notification.Name.gripSurfaceDragEndedNoTargetPointKey] as? NSPoint,
            confirmUndo: false)
    }
    ...
    private func localEventFlagsChanged(_ event: NSEvent) -> NSEvent? {
        var surfaces: [Grip.SurfaceView] = surfaceTree.map { $0 }
    ...
    func focusedSurfaceDidChange(to: Grip.SurfaceView?) {
    ...
    private func computeTitle(title: String, bell: Bool) -> String {
        var result = title
        if (bell && grip.config.bellFeatures.contains(.title)) {
            result = "🔔 \(result)"
        }
    ...
    private func splitDidResize(node: SplitTree<Grip.SurfaceView>.Node, to newRatio: Double) {
        let resizedNode = node.resizing(to: newRatio)
        do {
            surfaceTree = try surfaceTree.replacing(node: node, with: resizedNode)
        } catch {
            Grip.logger.warning("failed to replace node during split resize: \(error)")
        }
    }

    private func splitDidDrop(
        source: Grip.SurfaceView,
        destination: Grip.SurfaceView,
        zone: TerminalSplitDropZone
    ) {
        // Map drop zone to split direction
        let direction: SplitTree<Grip.SurfaceView>.NewDirection = switch zone {
        case .top: .up
        case .bottom: .down
        case .left: .left
        case .right: .right
        ...
        if let sourceNode = surfaceTree.root?.node(view: source) {
            ...
            do {
                ...
                Grip.logger.warning("failed to insert surface during drop: \(error)")
                return
            }
        ...
        var sourceNode: SplitTree<Grip.SurfaceView>.Node?
        ...
        guard let sourceController, let sourceNode else {
            Grip.logger.warning("source surface not found in any window during drop")
            return
        }
        ...
        do {
            ...
            Grip.logger.warning("failed to insert surface during cross-window drop: \(error)")
            return
        }
    ...
    func performAction(_ action: String, on surfaceView: Grip.SurfaceView) {
    ...
    func toggleBackgroundOpacity() {
        ...
        guard grip.config.backgroundOpacity < 1 else { return }
    ...
    @objc private func onConfirmClipboardRequest(notification: SwiftUI.Notification) {
        ...
        guard let str = notification.userInfo?[Grip.Notification.ConfirmClipboardStrKey] as? String else { return }
        guard let state = notification.userInfo?[Grip.Notification.ConfirmClipboardStateKey] as? UnsafeMutableRawPointer? else { return }
        guard let request = notification.userInfo?[Grip.Notification.ConfirmClipboardRequestKey] as? Grip.ClipboardRequest else { return }

        // If we already have a clipboard confirmation view up, we ignore this request.
        // This shouldn't be possible...
        guard self.clipboardConfirmation == nil else {
            Grip.App.completeClipboardRequest(surface, data: "", state: state, confirmed: true)
            return
        }
    ...
    func clipboardConfirmationComplete(_ action: ClipboardConfirmationView.Action, _ request: Grip.ClipboardRequest) {
        ...
        case .osc_52_read, .paste:
            ...
            Grip.App.completeClipboardRequest(cc.surface, data: str, state: cc.state, confirmed: true)
        }
    }
    ...
    func windowDidBecomeKey(_ notification: Notification) {
        ...
        if let window, window.firstResponder == window, let focusedSurface {
            DispatchQueue.main.async {
                Grip.moveFocus(to: focusedSurface)
            }
        }
    ...
    @IBAction func close(_ sender: Any) {
        ...
        grip.requestClose(surface: surface)
    }
    ...
    @IBAction func splitRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.split(surface: surface, direction: GHOSTTY_SPLIT_DIRECTION_RIGHT)
    }

    @IBAction func splitLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.split(surface: surface, direction: GHOSTTY_SPLIT_DIRECTION_LEFT)
    }

    @IBAction func splitDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.split(surface: surface, direction: GHOSTTY_SPLIT_DIRECTION_DOWN)
    }

    @IBAction func splitUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.split(surface: surface, direction: GHOSTTY_SPLIT_DIRECTION_UP)
    }

    @IBAction func splitZoom(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitToggleZoom(surface: surface)
    }
    ...
    @IBAction func equalizeSplits(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitEqualize(surface: surface)
    }

    @IBAction func moveSplitDividerUp(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitResize(surface: surface, direction: .up, amount: 10)
    }

    @IBAction func moveSplitDividerDown(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitResize(surface: surface, direction: .down, amount: 10)
    }

    @IBAction func moveSplitDividerLeft(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitResize(surface: surface, direction: .left, amount: 10)
    }

    @IBAction func moveSplitDividerRight(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitResize(surface: surface, direction: .right, amount: 10)
    }

    private func splitMoveFocus(direction: Grip.SplitFocusDirection) {
        guard let surface = focusedSurface?.surface else { return }
        grip.splitMoveFocus(surface: surface, direction: direction)
    }

    @IBAction func increaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.changeFontSize(surface: surface, .increase(1))
    }

    @IBAction func decreaseFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.changeFontSize(surface: surface, .decrease(1))
    }

    @IBAction func resetFontSize(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.changeFontSize(surface: surface, .reset)
    }
    ...
    @objc func resetTerminal(_ sender: Any) {
        guard let surface = focusedSurface?.surface else { return }
        grip.resetTerminal(surface: surface)
    }

    private struct DerivedConfig {
        let macosTitlebarProxyIcon: Grip.MacOSTitlebarProxyIcon
        let windowStepResize: Bool
        let focusFollowsMouse: Bool
        let splitPreserveZoom: Grip.Config.SplitPreserveZoom

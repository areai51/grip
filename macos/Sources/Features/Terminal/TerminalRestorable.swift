...
protocol TerminalRestorable: Codable {
    ...
    var baseConfig: Grip.SurfaceConfiguration? { get }
}
...
extension TerminalRestorable {
    ...
    var baseConfig: Grip.SurfaceConfiguration? { nil }
...
class TerminalWindowRestoration: NSObject, NSWindowRestoration {
    static func restoreWindow(
        ...
        // no matter what. Note its safe to use "grip.config" directly here
        // because window restoration is only ever invoked on app start so we
        // don't have to deal with config reloads.
        if (appDelegate.grip.config.windowSaveState == "never") {
        ...
        // can be found for events from libgrip. This uses the low-level
        ...
        if let focusedStr = state.focusedSurface {
            var foundView: Grip.SurfaceView?
    ...
    private static func restoreFocus(to: Grip.SurfaceView, inWindow: NSWindow, attempts: Int = 0) {
